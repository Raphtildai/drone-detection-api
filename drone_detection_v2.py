...
import os, subprocess, sys, json, math, time, random, shutil, zipfile, tempfile, hashlib
...
# Global audio extensions used across custom-dataset helpers and notebook upload flows
...
# Plots helpers
def show_and_save(fig, path=None, dpi=150):
    if path is not None:
...
def _show_plot_inline(fig):
    """Render plots in notebooks/Colab even when matplotlib uses Agg."""
    try:
        from IPython.display import display
        display(fig)
    except Exception:
...
def plot_training_logs(cfg: Config = None, save: bool = True):
    ...
    if save:
        ...
        show_and_save(fig, str(out))
    _show_plot_inline(fig)
    plt.close(fig)
...
def _per_frame_entropy(S: np.ndarray, min_energy_fraction: float = 0.01) -> np.ndarray:
    ...
    Only frames whose total energy >= min_energy_fraction * max_frame_energy
    contribute; the rest are set to NaN so callers can use np.nanmedian to
    ignore silent frames.

    Key insight: a short horn burst in a 3-second clip has ~82% silent frames.
    np.percentile(frame_energy, 25) == 0 in that case, so a percentile gate
    is useless. An absolute gate relative to the loudest frame always works.
    """
...
def _harmonic_comb_score(y: np.ndarray, sr: int,
                        f0_min: float = 80.0, f0_max: float = 350.0,
                        n_harmonics: int = 6) -> float:
  """
  Score how well the spectrum matches a drone harmonic comb.
  Sweeps a candidate F0 from f0_min to f0_max and for each checks how much
  energy sits at its harmonics vs the background.
  Returns value in [0, 1]: higher = more drone-like harmonic structure.
  """
  S = np.abs(librosa.stft(y, n_fft=2048, hop_length=512))
  S_mean = S.mean(axis=1)                          # average magnitude per bin
  ...
  for f0 in np.linspace(f0_min, f0_max, 30):
      ...
      for k in range(1, n_harmonics + 1):
          ...
          w   = max(1, int(20 / (freqs[1] - freqs[0])))  # ±20 Hz window
          harmonic_energy += float(S_mean[max(0,idx-w):idx+w+1].max())
      total_energy = float(S_mean.sum()) + 1e-10
      score = harmonic_energy / (total_energy * n_harmonics) * n_harmonics
      if score > best:
          best = score
...
def _impulsiveness(y: np.ndarray) -> float:
  """
  Crest factor: high = impulsive (car horn punch).
  Return normalised score where impulsive signals score LOW for drone.
  """
  rms  = float(np.sqrt(np.mean(y**2)) + 1e-8)
  peak = float(np.max(np.abs(y)) + 1e-8)
  cf   = peak / rms          # crest factor; siren/horn often > 6
  return float(np.clip(cf, 1.0, 10.0))
...
def heuristic_detect(audio: np.ndarray, cfg=None) -> dict:
    """
    v15 patch: absolute-energy-gated per-frame entropy.

    Correctly vetoes short tonal pulses and swept sirens.
    Correctly passes drone harmonic combs.
    """
    cfg = cfg or config
    ...
    try:
        ...
        rolloff   = float(np.mean(librosa.feature.spectral_rolloff(
                            S=S, sr=cfg.SR, roll_percent=0.85)))
    ...
    # Veto 2: very high crest factor + moderately narrow
    ...
    f0_score        = (1.0  if  80.0 <= f0_med <= 350.0
                  else 0.3  if  50.0 <= f0_med <   80.0
                  else 0.0)
    ...
    rolloff_score   = 1.0 if 300.0 <= rolloff  <= 8000.0 else 0.4
    bandwidth_score = 1.0 if 150.0 <= bandwidth <= 4000.0 else 0.5
    entropy_score   = float(np.clip((median_ent - 0.35) / 0.35, 0.0, 1.0))
    comb_score      = float(np.clip(comb * 3.0, 0.0, 1.0))

    score = (0.14 * energy_score
            + 0.16 * f0_score
            + 0.10 * voiced_score
    ...
    return {
        ...
        "features": {
            "rms_db":               rms_db,
            "median_frame_entropy": median_ent,
            "p10_entropy":          p10_ent,
            "crest_factor":         cf,
            "comb_score":           comb,
...
_RMS_FLOOR_DB = -45.0  # segments below this are near-silent noise floor

def detect(channels, cfg=None, use_hybrid: bool = True) -> dict:
    cfg = cfg or config
    y0 = AudioProcessor(cfg).pad_or_truncate(
        np.asarray(channels[0], dtype=np.float32)
    )
    rms_db = float(20 * math.log10(float(np.sqrt(np.mean(y0 ** 2))) + 1e-8))

    # Gate: return immediately for near-silent audio
    ...
    mel0 = AudioProcessor(cfg).mel(y0)
    m0   = torch.tensor(
...
if __name__ == "__main__":
    print("🚁 Drone Detection & Localization v15")
    print("=" * 65)
...
def _copy_tree_audio(src: Path, dst: Path, exts=AUDIO_EXTS):
    if not src.exists():
        return 0
    dst.mkdir(parents=True, exist_ok=True)
    n = 0
...
def upload_custom_dataset_artifacts(cfg: Config = None, extract_dir: str = None, accept_zip: bool = True):
    cfg = _ensure_custom_dataset_cfg(cfg or config)
    if not running_in_colab_runtime():
        raise RuntimeError("upload_custom_dataset_artifacts() is intended for Google Colab. Use a local folder path outside Colab.")
...
def normalize_custom_dataset_root(cfg: Config = None, dataset_root: str = None, zip_path: str = None,
                                  extract_dir: str = None, include_background_pool_as_non_drone: bool = None,
    ...
    candidates = [base_in]
    if base_in.is_dir():
        candidates.extend([c for c in sorted(base_in.iterdir()) if c.is_dir()])
    chosen = None
    for cand in candidates:
        if (cand / cfg.CUSTOM_DATASET_CLEAN_SUBDIR).exists() or (cand / cfg.CUSTOM_DATASET_TRAIN_SUBDIR).exists() or (cand / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR).exists():
            chosen = cand
            break
    if chosen is None:
        raise FileNotFoundError(f"No expected custom dataset structure found under {base_in}")
    normalized = Path(cfg.LOCAL_BASE) / "normalized_custom_dataset"
    if normalized.exists():
        shutil.rmtree(normalized)
    normalized.mkdir(parents=True, exist_ok=True)
    for sub in [cfg.CUSTOM_DATASET_CLEAN_SUBDIR, cfg.CUSTOM_DATASET_TRAIN_SUBDIR, cfg.CUSTOM_DATASET_VAL_SUBDIR, cfg.CUSTOM_DATASET_BGPOOL_SUBDIR]:
        (normalized / sub).mkdir(parents=True, exist_ok=True)
    clean_srcs = []
    if prefer_manual_clean and (chosen / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR).exists():
        clean_srcs.append(chosen / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR)
    if (chosen / cfg.CUSTOM_DATASET_CLEAN_SUBDIR).exists():
        clean_srcs.append(chosen / cfg.CUSTOM_DATASET_CLEAN_SUBDIR)
    for src in clean_srcs:
        _copy_tree_audio(src, normalized / cfg.CUSTOM_DATASET_CLEAN_SUBDIR)
    if (chosen / cfg.CUSTOM_DATASET_TRAIN_SUBDIR).exists():
        _copy_tree_audio(chosen / cfg.CUSTOM_DATASET_TRAIN_SUBDIR, normalized / cfg.CUSTOM_DATASET_TRAIN_SUBDIR)
    if (chosen / cfg.CUSTOM_DATASET_VAL_SUBDIR).exists():
        _copy_tree_audio(chosen / cfg.CUSTOM_DATASET_VAL_SUBDIR, normalized / cfg.CUSTOM_DATASET_VAL_SUBDIR)
    if (chosen / cfg.CUSTOM_DATASET_BGPOOL_SUBDIR).exists():
        _copy_tree_audio(chosen / cfg.CUSTOM_DATASET_BGPOOL_SUBDIR, normalized / cfg.CUSTOM_DATASET_BGPOOL_SUBDIR)
    ...
    cfg.CUSTOM_DATASET_ROOT = str(normalized)
    cfg.CUSTOM_DATASET_ENABLED = True
    print(f"✅ Normalized custom dataset ready at {normalized}")
    describe_custom_dataset(cfg, normalized)
    return str(normalized)


def describe_custom_dataset(cfg: Config = None, dataset_root: str = None):
    cfg = _ensure_custom_dataset_cfg(cfg or config)
    root = Path(dataset_root or cfg.CUSTOM_DATASET_ROOT)
    if not root.exists():
        print(f"⚠️ Custom dataset root not found: {root}")
        return {}
    stats = {}
    mapping = {
        'clean': root / cfg.CUSTOM_DATASET_CLEAN_SUBDIR,
        'train_drone': root / cfg.CUSTOM_DATASET_TRAIN_SUBDIR,
        'val_drone': root / cfg.CUSTOM_DATASET_VAL_SUBDIR,
        'background_pool': root / cfg.CUSTOM_DATASET_BGPOOL_SUBDIR,
        'manual_clean': root / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR,
    }
...
def import_custom_builder_dataset(cfg: Config = None, builder_root: Path = None, force: bool = False,
    ...
    builder_manual_clean = builder_root / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR
    ...
    if not clean_files and builder_manual_clean.exists():
        clean_files = _list_audio_files(builder_manual_clean, AUDIO_EXTS)
...
def train_detection(cfg: Config = None, epochs: int = None, resume: bool = True,
    ...
    print("=" * 70)
    print("  STAGE 1 — Detection Model (real + optional external + custom + mixed + synthetic)")
    print("=" * 70)

    if download_builtin_detection_dataset:
        DroneAudioDatasetManager(cfg).prepare()
    else:
        print("ℹ️  Skipping built-in DroneAudioDataset download/preparation by request.")
        for split in ["train", "val", "test"]:
            for label in ["drone", "non_drone"]:
    ...
    if download_external_audio:
        ...
        print("ℹ️  Skipping external audio scraping/download by request.")
...
def configure_custom_dataset(cfg: Config = None, dataset_root: str = None, zip_path: str = None,
                             extract_dir: str = None, include_background_pool_as_non_drone: bool = True,
                             prefer_manual_clean: bool = True):
    cfg = _ensure_custom_dataset_cfg(cfg or config)
    root = normalize_custom_dataset_root(cfg, dataset_root=dataset_root, zip_path=zip_path,
                                         extract_dir=extract_dir,
                                         include_background_pool_as_non_drone=include_background_pool_as_non_drone,
                                         prefer_manual_clean=prefer_manual_clean)
    cfg.CUSTOM_DATASET_ENABLED = True
    cfg.CUSTOM_DATASET_ROOT = root
    return root


def quickstart_notebook_setup(cfg: Config = None, dataset_root: str = None, zip_path: str = None,
    ...
    print(f"   CUSTOM_DATASET_ROOT                : {cfg.CUSTOM_DATASET_ROOT}")
...
# ══════════════════════════════════════════════════════════════════════════════
# v16b — Notebook template + stricter uploaded dataset validator
# ══════════════════════════════════════════════════════════════════════════════

def validate_custom_dataset_structure(cfg: Config = None, dataset_root: str = None,
                                      require_any_positive: bool = True,
                                      min_positive_files: int = 1,
                                      min_background_files: int = 0,
                                      verbose: bool = True):
    """
    Validate the structure of an uploaded / extracted custom dataset.

    Accepted positive sources:
      - clean_drone_segments/
      - manual_clean/clean_drone_sections/
      - train/drone/
      - val/drone/
      - test/drone/   (optional)

    Accepted optional negative/background source:
      - background_pool/

    Returns a dict with counts and validation flags.
    """
    cfg = _ensure_custom_dataset_cfg(cfg or config)
    root = Path(dataset_root or cfg.CUSTOM_DATASET_ROOT)
    if not root.exists():
    ...
    mapping = {
        "clean_drone_segments": root / cfg.CUSTOM_DATASET_CLEAN_SUBDIR,
        "manual_clean_sections": root / cfg.CUSTOM_DATASET_MANUAL_CLEAN_SUBDIR,
        "train_drone": root / cfg.CUSTOM_DATASET_TRAIN_SUBDIR,
        "val_drone": root / cfg.CUSTOM_DATASET_VAL_SUBDIR,
    ...
    positive_count = (
        counts["clean_drone_segments"]
        + counts["manual_clean_sections"]
        + counts["train_drone"]
        + counts["val_drone"]
        + counts["test_drone"]
    )
    ...
    warnings_list = []
    if require_any_positive and positive_count < int(min_positive_files):
        warnings_list.append(
            f"Not enough positive drone files found. Required >= {min_positive_files}, found {positive_count}."
        )
    if background_count < int(min_background_files):
        warnings_list.append(
            f"Not enough background_pool files found. Required >= {min_background_files}, found {background_count}."
        )

    preferred_positive_root = None
    if counts["manual_clean_sections"] > 0:
        preferred_positive_root = str(mapping["manual_clean_sections"])
    elif counts["clean_drone_segments"] > 0:
        preferred_positive_root = str(mapping["clean_drone_segments"])
    elif counts["train_drone"] > 0:
        preferred_positive_root = str(mapping["train_drone"])
...
def get_notebook_setup_template(cfg: Config = None, dataset_root: str = None,
                                zip_path: str = None,
                                include_background_pool_as_non_drone: bool = True,
                                download_builtin_detection_dataset: bool = False,
                                download_external_audio: bool = False):
    """
    Return a ready-to-paste Colab/local notebook setup snippet.
    """
    cfg = _ensure_custom_dataset_cfg(cfg or config)
    dataset_root_repr = repr(dataset_root) if dataset_root else "None"
    zip_path_repr = repr(zip_path) if zip_path else "None"
    return f"""# === Custom dataset setup ===
# Option A: upload files/zip in Colab
# uploaded_root = upload_custom_dataset_artifacts(config)

# Option B: point to an existing folder or zip
DATASET_ROOT = {dataset_root_repr}
ZIP_PATH = {zip_path_repr}

root = None
if ZIP_PATH:
    root = configure_custom_dataset(
        config,
        zip_path=ZIP_PATH,
        include_background_pool_as_non_drone={bool(include_background_pool_as_non_drone)},
    )
elif DATASET_ROOT:
    root = configure_custom_dataset(
        config,
...
quickstart_notebook_setup(
    ...
    dataset_root=root,
    include_background_pool_as_non_drone={bool(include_background_pool_as_non_drone)},
    download_builtin_detection_dataset={bool(download_builtin_detection_dataset)},
    download_external_audio={bool(download_external_audio)},
)
...
#     import_custom_backgrounds_as_non_drone={bool(include_background_pool_as_non_drone)},
#     download_builtin_detection_dataset={bool(download_builtin_detection_dataset)},
#     download_external_audio={bool(download_external_audio)},
# )
...
def print_notebook_setup_template(cfg: Config = None, dataset_root: str = None,
                                  zip_path: str = None,
                                  include_background_pool_as_non_drone: bool = True,
                                  download_builtin_detection_dataset: bool = False,
                                  download_external_audio: bool = False):
    template = get_notebook_setup_template(
        cfg=cfg,
        dataset_root=dataset_root,
        zip_path=zip_path,
        include_background_pool_as_non_drone=include_background_pool_as_non_drone,
...
def quickstart_notebook_setup_v2(cfg: Config = None, dataset_root: str = None, zip_path: str = None,
                                 include_background_pool_as_non_drone: bool = True,
                                 download_builtin_detection_dataset: bool = False,
                                 download_external_audio: bool = False,
                                 ...
                                 min_background_files: int = 0):
    """
    Slightly stricter notebook setup wrapper that can validate the uploaded dataset.
    """
    cfg = _ensure_custom_dataset_cfg(cfg or config)
