...
def safe_slug(name: str) -> str:
    ...
    return "".join(keep).strip("_") or "output_v2"
    ...
    def analyze_file(self, audio_path: Path) -> Dict:
        ...
        for i, (s, e) in enumerate(windows):
            ...
            if not is_silent and not speech_like:
                if score >= self.cfg.detect_threshold:
                    keep = True
                elif (score >= self.cfg.weak_threshold and res["features"].get("drone_band_ratio", 0.0) >= 0.18 and quality["active_ratio"] >= self.cfg.min_active_frame_ratio):
                    keep = True
            window_details.append({
                "window_index": i, "start_s": float(start_s), "end_s": float(end_s), "score": score,
                "quality": quality, "speech_like": bool(speech_like), "is_silent": bool(is_silent),
                "kept": bool(keep), "features": res["features"],
            })
            if keep:
                candidate_regions.append({"start_s": float(start_s), "end_s": float(end_s), "max_score": score, "mean_score": score, "windows": 1})
        merged = merge_regions(candidate_regions, max_gap_sec=self.cfg.merge_gap_sec, min_duration_sec=self.cfg.min_segment_sec)
        final_segments = []
        for region in merged:
            ...
            for cs, ce in split_long_region(start_s, end_s, max_chunk_sec=self.cfg.max_segment_sec, overlap_sec=self.cfg.segment_overlap_sec):
                final_segments.append({"start_s": float(cs), "end_s": float(ce), "duration_s": float(ce - cs), "score": float(region["max_score"])})
        return {"file": str(audio_path), "duration_s": float(total_s), "windows_total": len(window_details), "detected_regions": merged, "segments": final_segments, "window_details": window_details}

    def extract_segments_from_file(self, audio_path: Path, analysis: Dict, out_dir: Path, plot_dir: Optional[Path] = None) -> List[Path]:
        ensure_dir(out_dir)
        ...
        for i, seg in enumerate(analysis.get("segments", []), 1):
            ...
            if plot_dir is not None:
                ...
                save_waveform_and_mel_plot(audio_path=out_path, y=clip, sr=self.cfg.sr, out_path=plot_path, title=f"Extracted Segment: {out_path.stem}")
        return saved
    ...
    def extract_from_folder(self, source_root: Path, clean_out_dir: Path, debug_out_dir: Path, recursive: bool = True) -> Dict:
        ...
        for fpath in audio_files:
            try:
                ...
                saved = self.extract_segments_from_file(audio_path=fpath, analysis=analysis, out_dir=clean_out_dir, plot_dir=debug_out_dir / "segment_plots")
                ...
                if not saved and self.cfg.use_source_full_file_as_fallback_positive:
                    ...
                    if len(y) >= int(self.cfg.min_clip_sec * self.cfg.sr):
                        ...
                        segment_manifest.append({
                            "segment_path": str(out_path), "segment_name": out_path.name, "source_file": str(fpath),
                            "source_group": src_group, "source_sha1": file_sha1(fpath), "segment_sha1": file_sha1(out_path),
                            "fallback_full_file": True,
                        })
                        total_saved += 1
                print(f"🎯 {fpath.name}: windows={analysis['windows_total']} segments={len(analysis.get('segments', []))} saved={len(saved)}")
            except Exception as e:
        ...
        print(f"\n✅ Extraction complete: files={len(audio_files)}, segments={total_segments}, saved={total_saved}")
            ...
            try:
                ...
                if not clean_target.exists():
                    ...
                    manifest_rows.append({
                        "kind": "clean", "source_clean": str(drone_path), "output_v2": str(clean_target), "split": split,
                ...
                for j in range(self.cfg.augments_per_segment):
                    ...
                    manifest_rows.append({
                        "kind": "augmented", "source_clean": str(drone_path), "background": str(bg_path), "output_v2": str(out_path),
...
def get_project_dirs(base_dir: Optional[Path] = None) -> Dict[str, Path]:
    ...
    dirs = {
        ...
        "output": base_dir / "output_v2",
        "clean": base_dir / "output_v2" / "clean_drone_segments",
        "debug": base_dir / "output_v2" / "analysis_debug",
        "background_pool": base_dir / "output_v2" / "background_pool",
        "train": base_dir / "output_v2" / "train" / "drone",
        "val": base_dir / "output_v2" / "val" / "drone",
        "test": base_dir / "output_v2" / "test" / "drone",
        "plots": base_dir / "output_v2" / "plots",
        "source_plots": base_dir / "output_v2" / "plots" / "source",
        "segment_plots": base_dir / "output_v2" / "plots" / "segments",
        "mix_plots": base_dir / "output_v2" / "plots" / "mixes",
        "manifest": base_dir / "output_v2" / "augmentation_manifest.json",
    }
...
def run_builder(source_root=None, backgrounds_root=None, output_root=None, threshold=0.52, weak_threshold=0.34, window_sec=1.25, hop_sec=0.35, augments_per_segment=4, val_frac=0.15, test_frac=0.10, freesound_api_key: str = "", use_urbansound8k: bool = False, urbansound8k_auto_download: bool = False, use_synth_fallback: bool = True, include_test_split: bool = True):
    ...
    if output_root is not None:
        ...
        for p in [clean_dir, debug_dir, bg_pool_dir, train_dir, val_dir, test_dir, plots_dir, mix_plots_dir]:
            ensure_dir(p)
    else:
        output_root = dirs["output_v2"]
    ...
    print("=" * 70)
    print("  CUSTOM DRONE DATASET BUILDER v2")
    print("=" * 70)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Standalone custom drone sound extractor and augmenter (group-safe v2)")
    ...
    parser.add_argument("--output_v2", help="Output root folder")
    ...
    if args.source and args.backgrounds and args.output:
        return run_builder(source_root=args.source, backgrounds_root=args.backgrounds, output_root=args.output, **common_kwargs)
    if running_in_notebook():
        dirs = get_project_dirs()
        return run_builder(source_root=dirs["source"], backgrounds_root=dirs["backgrounds"], output_root=dirs["output_v2"], **common_kwargs)
