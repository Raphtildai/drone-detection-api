# -*- coding: utf-8 -*-
"""
dunakeszi_segment_analysis_inference.py
──────────────────────────────────────────────────────────────────────────────
...
def _match_mic_files(
    ...
    1. Directory mode (original behavior)
       mic_inputs = [dir1, dir2, dir3]

    2. File mode (NEW behavior)
       mic_inputs = [file_ch0, file_ch1, file_ch2]
       OR a single directory containing *_ch0.wav, *_ch1.wav, *_ch2.wav
    """
    exts = {e.lower() for e in file_ext}

    # ─────────────────────────────────────────────
    # CASE 1: user passed 3 FILES directly
    # ─────────────────────────────────────────────
    if all(Path(p).is_file() for p in mic_inputs):
        if len(mic_inputs) != 3:
            raise ValueError("Exactly 3 mic files required.")

        return [[Path(p) for p in mic_inputs]]

    # ─────────────────────────────────────────────
    # CASE 2: user passed ONE directory with ch0/ch1/ch2
    # ─────────────────────────────────────────────
    if len(mic_inputs) == 1 and Path(mic_inputs[0]).is_dir():
        d = Path(mic_inputs[0])
        files = [f for f in d.iterdir() if f.suffix.lower() in exts]

        groups = {}
        for f in files:
            base = re.sub(r'_ch[012]$', '', f.stem)  # strip only at end of stem
            if base not in groups:
                groups[base] = {}
            for ch_idx in [0, 1, 2]:
                if f'_ch{ch_idx}' in f.stem:
                    groups[base][ch_idx] = f
                    break

        triplets = []
        for base, chs in groups.items():
            if all(k in chs for k in [0, 1, 2]):
                triplets.append([chs[0], chs[1], chs[2]])  # one triplet: [ch0, ch1, ch2]
            else:
                print(f"⚠️ Missing channels for {base}, skipping")

        if not triplets:
            raise FileNotFoundError("No valid ch0/ch1/ch2 triplets found")

        if sort_by_name:
            triplets = sorted(triplets, key=lambda t: t[0].name)

        return triplets

    # ─────────────────────────────────────────────
    # CASE 3: original directory-per-mic mode
    # ─────────────────────────────────────────────
    assert len(mic_inputs) == 3, "mic_files must contain 3 directories."
    dirs = [Path(d) for d in mic_inputs]
    ...
    for ref in ref_files:
        ...
        for d in dirs[1:]:
            match = d / ref.name
            if not match.exists():
                print(f"⚠️ Missing {ref.name} in {d}")
                ok = False
                break
            group.append(match)
        if ok:
    ...
    if not triplets:
        raise FileNotFoundError("No matching triplets found")
...
def _infer_timestamp(filename: str, fallback_idx: int, cfg) -> float:
    ...
    # Pattern get the training segment number (e.g. seg_000.wav → 0, seg_123.wav → 123)
    # Pattern: leading digits (e.g. 001, 000500)
    # m = re.match(r"^(\d+)", name)
    # # Pattern get the training segment number (e.g. seg_000.wav → 0, seg_123.wav → 123)
    m = re.search(r"seg[_-](\d+)", name)
    if m:
        val = int(m.group(1))
...
def _plot_polar_azimuth(rows: List[dict], save_path: Optional[Path]):
    ...
    # Collect azimuths (filter out None and NaN)
    az_list = []
    for r in rows:
        if r["detected"]:
            az = r.get("azimuth_deg")
            if az is not None and not (isinstance(az, float) and math.isnan(az)):
                az_list.append(az)
    ...
    rads = np.radians([-a for a in az_list])
    ...
    # Plot bars with explicit color and alpha
    ax.bar(centers, counts, width=width,
           color=_STYLE["accent"], alpha=0.8, 
           edgecolor=_STYLE["bg"], linewidth=0.5)
    
    # Set radial limits to show bars properly
    max_count = max(counts) if len(counts) > 0 else 1
    ax.set_ylim(0, max_count * 1.15)
    
    # Add radial grid and labels
    ax.set_yticks(np.linspace(0, max_count, 3))
    ax.set_yticklabels([f'{int(tick)}' for tick in ax.get_yticks()], 
                      fontsize=8, color=_STYLE["text"])
    
    # Add radial grid lines
    ax.yaxis.grid(True, color=_STYLE["grid"], alpha=0.3, linestyle='--')

    ax.set_theta_zero_location("N")
    ax.set_theta_direction(-1)
    ax.set_title(
        f"Azimuth distribution (N-up)\n({len(az_list)} detections)",
        pad=20, color=_STYLE["text"], fontweight="bold"
    )
    ax.grid(color=_STYLE["grid"], alpha=0.5, linestyle='-', linewidth=0.5)
    
    # Force axis to be visible
    ax.spines['polar'].set_visible(True)
    ax.spines['polar'].set_color(_STYLE["spine"])
...
def _plot_polar_subplot(ax, rows):
    """Helper function to create polar subplot for dashboard."""
    ax.cla()
    ax.set_facecolor(_STYLE["panel"])
    ax.tick_params(colors=_STYLE["text"])
    ...
    if az_list:
        ...
        # Use more bins for better resolution with clustered data
        n_bins = min(72, max(12, len(az_list) * 2))  # Adaptive bin count
        counts, edges = np.histogram(rads, bins=n_bins, range=(-np.pi, np.pi))
        centers = 0.5 * (edges[:-1] + edges[1:])
        width = edges[1] - edges[0]
        
        # Plot bars
        bars = ax.bar(centers, counts, width=width,
                      color=_STYLE["accent"], alpha=0.8, 
                      edgecolor=_STYLE["bg"], linewidth=0.5)
        ...
        if len(az_list) <= 12:
            for center, count in zip(centers, counts):
                if count > 0:
                    ax.text(center, count + 0.1, str(int(count)), 
                           ha='center', va='bottom', fontsize=7, color=_STYLE["text"])
    else:
        ax.text(0.5, 0.5, "No azimuth\ndata", transform=ax.transAxes,
                ha='center', va='center', color=_STYLE["muted"], fontsize=10)
    
    ax.set_theta_zero_location("N")
    ax.set_theta_direction(-1)
    ax.set_title(f"Azimuth (N-up)\n{len(az_list)} detections",
                 pad=15, color=_STYLE["text"], fontweight="bold", fontsize=10)
    ax.grid(color=_STYLE["grid"], alpha=0.5, linestyle='-', linewidth=0.5)
    ax.spines['polar'].set_visible(True)
    ax.spines['polar'].set_color(_STYLE["spine"])
...
# Local usage
# python dunakeszi_segment_analysis_inference.py --segment_dir /home/tildai/Desktop/Development/drone-detection-api/deployment/v2/dataset_builders/clean --n_mics 1